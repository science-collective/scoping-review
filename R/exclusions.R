...
# TODO: Use ggcohort to track what is removed?
  data |>
    ...
    exclude(title, "hysterectomy|surgery|surgical|angle glaucoma|fracture|abdomen|wound|colectomy|vein|resection|gastrectomy|urethroplasty") |>
    # Items related to healthcare or health study design
    exclude(title, "open( |-)(cohort|label|trial|radical|radiology|acute|study|weighing|registry)|rehab|neuroscience|rt-qpcr|randomi[zs]ed|neuronal|neurons") |>
    ...
    exclude(title, "resonator|nanoparticle|robotic|x-ray|open( |-)(universe|systems)|nanofluidic") |>
    ...
    exclude(title, "open( |-)(end(ed)?|issues|synthesis|water|air|leader(ship)?|domain|platform|monitoring|tennis|questions|charm|games|competition)|hunter")
}
