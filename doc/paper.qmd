...
In @Turoman2022 the authors describe in detail how the have set up a
laboratory whose members collaborate in an open way from study design to
dissemination. Here we highlight the role of group meetings as
opportunities to propose ideas and have an initial study design
proposed. These group meetings are usually started with a section of
interesting mistakes, which allows to share the issues, troubleshoot and
make the laboratory members comfortable with the fact that errors are
unavoidable in science. The feedback from these meetings might lead to
updates to a lab manual, which is a great tool for new members to start
engaging with the open practices of the lab. This coupled with a support
system by senior members of the laboratory allows, in theory, to ease
the learning curve of new members to adapt to open practices and
collaboration.

In the paper, authors describe their implementation of open practices in
each step of the scientific production. Here we focus on the aspects in
...
communicate between members and OSF for data deposition and uploading
pre-registered studies. They put special emphasis on folder structure,
which is consistent for all projects, and assigning the authorship in
this stage. For data analysis, the authors highlight the figure of the
data partner, a member of the laboratory not originally included in the
analyses that runs the analyses described in the pre-registered study
independently. If the results mismatch with the original ones meetings
with the lead analyst or all collaborators are organized to assess what
the issue might be. Additionally, this allows for insights that might
result in deviations of the original pre-registered study, which should
be included and noted in the final manuscript. Importantly, the role of
the data partner is described in the final manuscript. Finally, in the
dissemination step, they highlight that they might incorporate feedback
from presenting their analyses in conferences, which are often included
as additional non-preregistered analyses.

Finally, in the paper they also highlight relevant open practices that
might not enhance collaboration, but that we consider interesting
reviewing too. First, during pre-registration, they allow for papers to
...
they included soundness checks whenever possible. Third, the perform
