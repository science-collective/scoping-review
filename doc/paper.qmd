...
n_papers <- tar_read(records_title_only) %>% nrow() %>% as.numeric()

n_papers_title_review <- tar_read(records_after_title_exclusion) %>% nrow() %>% as.numeric()

n_papers_excluded_title_review <- n_papers - n_papers_title_review

n_titles_selected <- tar_read(titles_selected) %>% nrow() %>% as.numeric()

n_paper_full_text <- tar_read(abstracts_selected) %>% nrow() %>% as.numeric()

n_papers_selected_for_review <- read.csv(here("data", "article_characteristics.csv"), header = TRUE) %>% nrow() %>% as.numeric() 

#get table of study characteristics 
article_characteristics <- read.csv(here("data", "article_characteristics.csv"))

n_guides <- article_characteristics %>% filter(paper_type == "guide") %>% nrow() %>% as.numeric()

n_examples <- article_characteristics %>% filter(paper_type == "example") %>% nrow() %>% as.numeric()

n_surveys <- article_characteristics %>% filter(paper_type == "survey") %>% nrow() %>% as.numeric()

n_org <- article_characteristics %>% filter(str_detect(themes, "organization")) %>% nrow() %>% as.numeric()

n_tools <- article_characteristics %>% filter(str_detect(themes, "tools")) %>% nrow() %>% as.numeric()

n_education <- article_characteristics %>% filter(str_detect(themes, "education")) %>% nrow() %>% as.numeric()

n_researcher <- article_characteristics %>% filter(relevant_for == "researcher") %>% nrow() %>% as.numeric()

n_research_group <- article_characteristics %>% filter(relevant_for == "research group") %>% nrow() %>% as.numeric()

n_research_center <- article_characteristics %>% filter(relevant_for == "research center") %>% nrow() %>% as.numeric()

n_research_centers <- article_characteristics %>% filter(relevant_for == "multiple centers") %>% nrow() %>% as.numeric()

n_research_field <- article_characteristics %>% filter(relevant_for == "research field") %>% nrow() %>% as.numeric()

```

The search yielded a total of `r n_papers` papers from which we filtered
`r n_papers_excluded_title_review` based on key words. The remaining
`r n_papers_title_review` titles were independently screened by three
reviewers from which `r n_titles_selected` abstracts were screened. Of
these, `r n_paper_full_text` papers were assessed in full-text and
`r n_papers_selected_for_review` were included in the scoping review.
...
(Finndable, Accessible, Interoperable and Reusable) principles.
...
to improve open collaboration @bush_lessons_2022. First, they used
open-source code, then standardized data formats, developed analysis
pipelines, shared data openly and developed protocols for data
documentation. While the organizational transition to improve open
collaboration was described, little was mentioned about how this has
improved collaborative work at the research center.

Interestingly, the level of organization with most papers was the
multi-center level with two papers from ManyPrimates et al
(@manyprimates_collaborative_2019, @manyprimates_collaboration_2021) and
Grange et al. (@grange_ukrn_2023). In this organization level the most
urgent needs are: 1) identifying the needs for open collaboration and 2)
creating an infrastructure that allows those needs to be satisfied.
Indeed, Grange et al describes a systematic review as an exploration of
the needs for 11 UKRN centers to implement open collaborative practives.
In this paper they specially focus on tools and resources among the 11
...
first paper from ManyPrimates et al @manyprimates_collaborative_2019
describes the start of another multi-center collaboration, albeit from
different institutions. It is interesting to observe that in this
situation, the complex issue is to start the collaboration itself, which
was ignited thanks to a symposium and an email chain among future
collaborators. In contrast, ManyPrimates could quickly develop an open
collaborative structure with relative ease in comparison to changing an
already solid infrastructure, like in the case of Grange et al.
